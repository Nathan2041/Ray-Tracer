...
class BoundingBox {
	...
	public constructor(public readonly triangles: Triangle[]) {
		...
		let extent = new Vector3(this.max.x - this.min.x, this.max.y - this.min.y, this.max.z - this.min.z);
...
let hierarchy: BoundingBox  = new BoundingBox(triangles);
...
let uBoundingBoxes:    	WebGLUniformLocation = gl.getUniformLocation(program, 'u_boundingBoxes')!;
let uTriangles:        	WebGLUniformLocation = gl.getUniformLocation(program, 'u_triangles')!;
let uRay:              	WebGLUniformLocation = gl.getUniformLocation(program, 'u_ray')!;
...
let packed:     { triangles: WebGLTexture, boundingBoxes: WebGLTexture, boundingBoxesSize: number } = packElements(gl, hierarchy);
let rayTexture: WebGLTexture = makeTexture(gl, new Float32Array(rays.flatMap(ray => classToArray(ray))));
...
function render(time: number): void {
	...
	gl.uniform1i(uBoundingBoxes,     1);
	gl.uniform1i(uTriangles,         2);
	gl.uniform1i(uRay,               3);
	gl.uniform1i(uTriangleCount, packed.boundingBoxesSize);

	gl.activeTexture(gl.TEXTURE1);
	gl.bindTexture(gl.TEXTURE_2D, packed.boundingBoxes);
	gl.activeTexture(gl.TEXTURE2);
	gl.bindTexture(gl.TEXTURE_2D, packed.triangles);
	gl.activeTexture(gl.TEXTURE3);
	gl.bindTexture(gl.TEXTURE_2D, rayTexture);
...
function alignedHemisphereSample(normal: NormalizedVector3): NormalizedVector3 {
	...
	let sample = new NormalizedVector3(sinTheta * Math.cos(phi), sinTheta * Math.sin(phi), cosTheta);
...
function rayBVHIntersection(ray: Ray, node: BoundingBox | Triangle[]): HitInfo {
	if (Array.isArray(node)) { return rayTrianglesIntersection(ray, node) }
...
function classToArray(instantiation: Ray | StoredBoundingBoxNode | Triangle[]): number[] {
	let array: number[] = [];
	...
	if (typeof instantiation === "object" && instantiation !== null && "hits" in instantiation) {
		return [
			instantiation.origin.x, instantiation.origin.y, instantiation.origin.z,
			instantiation.direction.x, instantiation.direction.y, instantiation.direction.z
		];
	}

	array.push(
		instantiation.index,
		instantiation.min.x, instantiation.min.y, instantiation.min.z,
		instantiation.max.x, instantiation.max.y, instantiation.max.z
	);
...
function packElements(
	...
	triangles: WebGLTexture,
	boundingBoxes: WebGLTexture,
	boundingBoxesSize: number
} {

	let nodes: BoundingBox[] = [];
	...
	let nodeMap = new Map<BoundingBox, number>();
	nodes.forEach((n, i) => nodeMap.set(n, i));

	let triangleIndexPool: number[] = [];

	let bbData: number[] = [];

	for (let nodeIndex: number = 0; nodeIndex < nodes.length; nodeIndex++) {
		let node: BoundingBox = nodes[nodeIndex];
		...
		for (let side = 0 as const; side < 2; side++) {
			let child: BoundingBox | Triangle[] = node.children[side];

			if (Array.isArray(child)) {
				let start: number = triangleIndexPool.length;

				for (let tri of child) {
					triangleIndexPool.push(tri.index);
				}

				if (side === 0) {
					packed.child0Type  = 2;
					packed.child0Start = start;
					packed.child0Count = child.length;
				} else {
					packed.child1Type  = 2;
					packed.child1Start = start;
					packed.child1Count = child.length;
				}

			} else {

				let childIndex: number = nodeMap.get(child)!;

				if (side === 0) {
					packed.child0Type  = 1;
					packed.child0Start = childIndex;
					packed.child0Count = 1;
				} else {
					packed.child1Type  = 1;
					packed.child1Start = childIndex;
					packed.child1Count = 1;
				}
			}
		}
	...
	let triangleTextureData: number[] = [
		...classToArray(triangles),
		...triangleIndexPool
	];

	return {
		triangles: makeTexture(gl, new Float32Array(triangleTextureData)),
		boundingBoxes: makeTexture(gl, new Float32Array(bbData)),
		boundingBoxesSize: bbData.length
	};
}
// #endregion